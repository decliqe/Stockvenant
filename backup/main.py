...
from PyQt6.QtWidgets import (
    ...
    QGridLayout, QDoubleSpinBox, QMessageBox, QDateEdit, QCalendarWidget, QListWidget, QTableWidget, QTableWidgetItem,
    QHeaderView, QListWidgetItem, QDockWidget, QMainWindow, QWidget
)

from dataloader import DataManager, DataLoaderWidget
...
class Stockvenant(QMainWindow):
    ...
    def __init__(self):
        ...
        # Load dataset once
        dataset_path = Path(__file__).parent / 'data' / 'temp_merged.csv'
        try:
        ...
        # Initialize data manager from dataloader
        self.data_manager = DataManager()
        self.data_manager.auto_merge_enabled = True

        # Connect to data updates
        self.data_manager.data_updated.connect(self.on_new_data_available)
        self.data_manager.merge_completed.connect(self.on_data_merged)


        # --- Widgets ---
        # Create the data loader widget
        self.data_loader_widget = DataLoaderWidget(self.data_manager)

        # Add as a dock widget on the right side
    ...
    def on_new_data_available(self, symbol, data):
        """
        Called when new data is received for a symbol.

        Args:
            symbol: Stock ticker symbol
            data: DataFrame with Date and price columns
        """
        print(f"New data available for {symbol} - {len(data)} rows")
        # Update your stock list if needed
        self.refresh_stock_list()


    def on_data_merged(self, merged_df):
        """
        Called when multiple symbols are merged into framework format.

        Args:
            merged_df: Merged DataFrame ready to use in calculations
        """
        ...
        # Option 2: Save and reload
        temp_file = 'data/temp_merged.csv'
        self.data_manager.save_master_to_csv(temp_file)
        self.df = load_dataset(temp_file)
    ...
    def refresh_stock_list(self):
        """Refresh the stock list with newly loaded data."""
        ...
        for i in range(self.stock_list.count()):
            ...
            if item.checkState() == Qt.CheckState.Checked:
                selected_stocks.append(item.text())

        # Clear and repopulate
        self.stock_list.clear()

        # Repopulate with new stocks
